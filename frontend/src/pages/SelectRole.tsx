    ...
    <div className="flex min-h-screen bg-white items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="space-y-3">
          {roles.map((r) => {
            return (
              <button
                onClick={() => setRole(r)}
                className={`w-full text-sm rounded-xl px-2 py-3 font-medium capitalize transition
                ${
                  role === r
        ...
        <button
          disabled={!role}
          onClick={addRole}
          className={`w-full max-w-sm rounded-xl px-4 py-3 font-semibold transition
            ${
              !role
