...
export const approveIssue = async (req, res) => {
    try {
        const { id } = req.params;
        const issue = await Issue.findByIdAndUpdate(id, { status: "APPROVED", updatedAt: new Date() }, { new: true });
        if (!issue) {
            return res
                .status(404)
...
export const rejectIssue = async (req, res) => {
    try {
        const { id } = req.params;
        const issue = await Issue.findByIdAndUpdate(id, { status: "REJECTED", updatedAt: new Date() }, { new: true });
        if (!issue) {
            return res
                .status(404)
                .json({ success: false, message: "Issue not found" });
